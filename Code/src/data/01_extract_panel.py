...
# ---------------------------------------------------------------------------
# 1. Conexión  (variable de entorno; nunca hardcode)
# ---------------------------------------------------------------------------
DATABASE_URL = os.environ.get(
    "DATABASE_URL",
    # Fallback para desarrollo local (mismo que usó schema_discovery.py).
    # En producción, setear siempre la variable de entorno.
...
# ---------------------------------------------------------------------------
# 3. Validar que todas las columnas existen en la tabla
# ---------------------------------------------------------------------------
def validate_columns(engine, table: str, requested: list[str]) -> list[str]:
    ...
    missing = [c for c in requested if c not in db_cols]
    return missing
...
# 4. Extracción
# ---------------------------------------------------------------------------
def extract(engine, columns: list[str], table: str = "inclusion_financiera_clean") -> pd.DataFrame:
    cols_sql = ", ".join(f'"{c}"' for c in columns)
    query = f"SELECT {cols_sql} FROM {table} ORDER BY cve_mun, periodo_trimestre"
    with engine.connect() as conn:
...
def main():
    print("=" * 60)
    print("01_extract_panel — Extracción del panel analítico")
    print("=" * 60)

    # Validar columnas
    missing = validate_columns(engine, "inclusion_financiera_clean", ALL_COLS)
    if missing:
    ...
    # Extraer
    print(f"Extrayendo {len(cols_to_extract)} columnas …")
    df = extract(engine, cols_to_extract)
