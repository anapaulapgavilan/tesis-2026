...
from tesis_alcaldesas.config import OUTPUT_PAPER, OUTCOME_LABELS
...
def asinh_to_pct(delta: float, baseline_asinh: float = 0.0) -> float:
    """
    Aproximación: para asinh(y) ≈ ln(2y) cuando y >> 1,
    Δ asinh ≈ Δ ln(y) ≈ % cambio.

    Para valores cercanos a 0, asinh(y) ≈ y, así que Δ asinh ≈ Δ y.
    
    Retorna el cambio porcentual aproximado.
    Usamos la relación exacta: si asinh(y0) = a, entonces
      y0 = sinh(a), y1 = sinh(a + delta)
      pct_change = (y1 - y0) / y0 × 100
    
    Pero sin conocer el baseline, usamos la aproximación:
      |Δ asinh| ≈ |Δ ln(y)| para y grande → % change ≈ (exp(|delta|) - 1) × 100
    """
