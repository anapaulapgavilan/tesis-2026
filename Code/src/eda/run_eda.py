...
# Usar variable de entorno si existe; si no, patrón existente del repo
import os
...
def seccion_d(df: pd.DataFrame):
    ...
    for c in OUTCOME_COLS_M:
        short = c.replace("ncont_", "").replace("numtar_", "t_").replace("numcontcred_", "cr_")
        df_ts[f"{short}_m_pc"] = df_ts[c] * 10_000 / df_ts["pob_adulta_m"].replace(0, np.nan)
    for c in OUTCOME_COLS_H:
        short = c.replace("ncont_", "").replace("numtar_", "t_").replace("numcontcred_", "cr_")
        df_ts[f"{short}_h_pc"] = df_ts[c] * 10_000 / df_ts["pob_adulta_h"].replace(0, np.nan)
